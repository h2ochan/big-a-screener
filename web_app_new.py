...
with st.sidebar:
    ...
    drawdown_lookback_years = st.number_input("条件2回看年数", min_value=1, max_value=30, value=15, step=1)
    max_low_to_peak_pct = st.number_input("条件2 低点/高点 上限(%)", min_value=1, max_value=150, value=30, step=1)
    mid_window_years = st.number_input("条件3 中期窗口上限(年)", min_value=1, max_value=10, value=5, step=1)
    max_mid_range_pct = st.number_input("条件3 中期振幅上限(%)", min_value=10, max_value=1000, value=150, step=5)
    min_mid_trend_pct = st.number_input("条件3 中期趋势下限(%)", min_value=10, max_value=1000, value=100, step=5)
    recent_range_months = st.number_input("条件4/5 近期窗口(月)", min_value=3, max_value=24, value=6, step=1)
    max_range_last_months_pct = st.number_input("条件4 近6月振幅上限(%)", min_value=5, max_value=500, value=50, step=5)
    min_volume_ratio_pct = st.number_input("条件5 近6月量比下限(%)", min_value=10, max_value=2000, value=300, step=10)
    max_market_cap_yi = st.number_input("条件1 市值上限(亿元)", min_value=1, max_value=5000, value=200, step=10)
...
if run:
    ...
    if int(max_symbols) > 0:
        cmd.extend(["--max-symbols", str(int(max_symbols))])
    if refresh_cache:
    ...
    if run_mode.startswith("全量模式"):
        with st.spinner("筛选中..."):
            try:
                ...
                st.error(
                    f"运行超时（>{int(timeout_seconds)}秒）。建议把“最多扫描股票数”调小（如 500~1000），"
                    "并把并发调到 2~4 再试。"
                )
                st.stop()
        ...
        total_symbols = int(max_symbols) if int(max_symbols) > 0 else 3000
        step = int(batch_size)
        ...
        if outputs:
            st.subheader("命令输出（分批）")
            st.code("\n\n".join(outputs))

        if frames:
            all_df = pd.concat(frames, ignore_index=True)
            all_df = all_df.drop_duplicates(subset=["code"], keep="first")
            final_df = rank_results(all_df, int(top_n))
            final_df.to_csv(RESULT, index=False)
            st.subheader(f"结果（{len(final_df)} 条）")
            st.dataframe(fmt_df(final_df), use_container_width=True)
            st.download_button("下载CSV", RESULT.read_bytes(), file_name="screen_results_new.csv", mime="text/csv")
        else:
